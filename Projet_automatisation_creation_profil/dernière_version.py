...
def creer_utilisateur_AD(conn, prenom, nom, trigramme, mot_de_passe, service, adresse, code_postal, email):
    try:
        ...
        print(f"Modification du mot de passe pour {prenom} {nom}.")
        conn.extend.microsoft.modify_password(user_dn, mot_de_passe)
        print(f"Mot de passe modifié avec succès pour {prenom} {nom}.")
        
        conn.modify(user_dn, {'userAccountControl': [('MODIFY_REPLACE', 512)]})
        
        print(f"Utilisateur {prenom} {nom} activé avec succès.")
        
        print(f"Création réussie pour {prenom} {nom}. Résultat : {conn.result}")
        return conn.result
    except Exception as e:
        print(f"Une erreur s'est produite lors de la création de l'utilisateur {prenom} {nom} : {e}")
        return None


def main():
    server_address = 'DCSRV.groupe-lepine.com'
    port = 389  #Port perso : 3268
